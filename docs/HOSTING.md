...
| Original uploaded PDFs | Cloudflare R2, bound as `BUCKET` |
| Computer, iPad and iPhone access | The same HTTPS app URL; optional browser installation |

Vercel and Supabase are not dependencies of this application. The existing hosting already provides the runtime, database and file storage it needs. GitHub stores the source; pushing code there does not move project data, upload drawings, or deploy a new version of the live app.
...
The current platform can inject trusted `oai-authenticated-user-*` identity headers. When hosting outside Sites, replace that identity integration with verified authentication or remove that header-based identity path. Do not accept those headers directly from public requests. Guest sessions and review links must keep their server-side ownership checks.

Moving to Vercel with Supabase would require porting the Worker-specific backend, replacing D1 database queries and R2 file operations, deciding on authentication, and migrating existing data. Connecting the repository to those services alone will not make this version work there. No such migration is needed to use the currently hosted app.
...
Anonymous owner sessions are tied to the browser, last 30 days and are lost when its cookies are cleared. Installing the app does not create a permanent owner account. Permanent account recovery is separate future work.
