...
export const GET=withSession(async(req)=>{try{
 ...
 return json({members:[{id:'owner',name:a.role==='owner'?a.name:'Project administrator',email:a.role==='owner'?a.email:'',profession:a.project.owner_profession,access:'editor',joined:true,owner:true},...rows.map(m=>({...m,user_id:undefined,joined:!!m.user_id}))]});
...
export const POST=withSession(async(req)=>{try{
 ...
 if(b.action==='remove'){await db().prepare('DELETE FROM project_members WHERE id=? AND project_id=?').bind(z.string().parse(b.id),a.project.id).run();return json({ok:true})}
 const job=z.enum(['architect','owner','builder']).parse(b.profession);
