...
async function pending(token:string){
 ...
 WHERE l.token_hash=? AND l.used=0 AND l.expires>? AND m.enabled=1`).bind(await hash(token),Date.now()).first<any>();
...
export const POST=withSession(async req=>{try{
 ...
 if(b.action==='inspect'||b.action==='complete'){
  ...
  const claimed=await db().prepare('UPDATE account_links SET used=1 WHERE token_hash=? AND used=0 AND expires>? AND EXISTS(SELECT 1 FROM project_members m WHERE m.id=account_links.member_id AND m.project_id=account_links.project_id AND m.email=account_links.email AND m.enabled=1)').bind(row.token_hash,Date.now()).run();
 ...
 const member=await db().prepare('SELECT id,email,name,profession FROM project_members WHERE project_id=? AND id=? AND enabled=1').bind(a.project.id,z.string().parse(b.memberId)).first<any>();
