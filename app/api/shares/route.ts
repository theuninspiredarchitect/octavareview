...
import {z} from 'zod';
export async function GET(req:Request){try{const a=await access(req,new URL(req.url).searchParams.get('project'));if(a.role!=='owner')fail('Only the owner can manage review links.',403);return json((await db().prepare('SELECT id,label,role,created FROM shares WHERE project_id = ? AND revoked = 0 ORDER BY created DESC').bind(a.project.id).all()).results)}catch(e){return error(e)}}
export async function POST(req:Request){try{
 ...
 const role=z.enum(['client','editor']).parse(b.role),sheetIds=z.array(z.string()).min(1).max(500).parse(b.sheetIds);
 ...
 await db().prepare('INSERT INTO shares (id,project_id,token_hash,role,sheet_ids,label,created,revoked) VALUES (?,?,?,?,?,?,?,0)').bind(id,a.project.id,await hash(token),role,JSON.stringify(sheetIds),label,created).run();
 return json({id,token,role,label,created});
