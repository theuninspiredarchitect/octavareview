...
import {z} from 'zod';
async function handleGET(req:Request){try{const a=await access(req,new URL(req.url).searchParams.get('project'));if(a.role!=='owner')fail('Only the owner can manage review links.',403);return json((await db().prepare('SELECT id,label,role,profession,created FROM shares WHERE project_id = ? AND revoked = 0 ORDER BY created DESC').bind(a.project.id).all()).results)}catch(e){return error(e)}}
async function handlePOST(req:Request){try{
 ...
 await db().prepare('INSERT INTO shares (id,project_id,token_hash,role,sheet_ids,label,created,profession,revoked) VALUES (?,?,?,?,?,?,?,?,0)').bind(id,a.project.id,await hash(token),role,JSON.stringify(sheetIds),label,created,profession).run();
