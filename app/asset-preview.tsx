...
export default function AssetPreview({file,projectId,headers,large=false}:{file:Pick<ProjectAsset,'id'|'source'|'mime'|'name'|'size'>;projectId:string;headers:Record<string,string>;large?:boolean}){
 ...
 useEffect(()=>{
  ...
  (async()=>{
   ...
   if(!large&&file.mime.startsWith('image/')){
    const path='/api/thumbnails?project='+projectId+'&id='+file.id,r=await fetch(path,{headers:auth.current,signal:controller.signal});
    if(r.ok)blob=await r.blob();
    else if(r.status===404&&file.size<=25*1024*1024){
     const original=await fetch(assetPath(projectId,file),{headers:auth.current,signal:controller.signal});if(!original.ok)throw Error();const thumbnail=await makeThumbnail(await original.blob());if(!thumbnail)throw Error();blob=thumbnail;
     void fetch(path,{method:'POST',headers:auth.current,body:thumbnail,signal:controller.signal}).catch(()=>{});
    }else throw Error();
   }else{const r=await fetch(assetPath(projectId,file),{headers:auth.current,signal:controller.signal});if(!r.ok)throw Error();blob=await r.blob()}
   if(live){url=URL.createObjectURL(blob);setSrc(url)}
  })().catch(()=>{if(live)setError(true)});
  return()=>{live=false;controller.abort();if(url)URL.revokeObjectURL(url)};
 },[file.id,projectId,large,visible]);
