...
export type Profession='architect'|'owner'|'builder';
export const professionNames:Record<Profession,string>={architect:'Architect',owner:'Owner',builder:'Builder'};
...
export type Sheet = {id:string;type:'sheet';code:string;name:string;revision:string;page:number;width:number;height:number;fileId?:string;groupId:string;sample?:string;calibration?:number;unit?:string;audience?:Audience;created:string;version?:number};
export type Markup = {id:string;type:'markup';sheetId:string;kind:'pen'|'line'|'arrow'|'rect'|'ellipse'|'measure'|'area'|'text';points:Point[];color:string;width:number;text?:string;textSize?:number;areaShape?:'rectangle'|'polygon';precision?:number;editable?:boolean;visibility:'internal'|'client';audience?:Audience;authorRole?:Profession;creatorId?:string;author:string;created:string;version?:number};
export type Comment = {id:string;type:'comment';sheetId:string;taskId:string;text:string;author:string;authorRole?:Profession;creatorId?:string;attachments?:Attachment[];audience?:Audience;created:string;visibility:'internal'|'client';version?:number};
...
export type ReviewData={project:Project;records:RecordItem[];role:'owner'|'editor'|'client';profession?:Profession;userId?:string;name:string;guest?:boolean;projects:Project[]};
