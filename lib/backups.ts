...
export const backupTables=['records','files','attachments','plan_folders','project_documents','photo_groups','photo_group_items','photo_likes','project_members','specification_groups','specifications','meetings','meeting_reports','document_drawings'] as const;
...
export async function startRestore(a:Access,id:string){
 ...
 for(const table of ['records','files','attachments','plan_folders','project_documents','photo_groups','specification_groups','specifications','meetings','meeting_reports','document_drawings'])for(const row of m.tables[table]||[])if(!map[row.id])map[row.id]=crypto.randomUUID();
